    ...
    export default {
        ...
        methods:{
          ...
          getdata() {  //获取数据
            ...
            this.$axios.post("menu/queryAuthorMenu.action",params).then(function (result) {  //成功  执行then里面的方法
          ...
          getRoleMenu(row,column,event){
            ...
            this.$axios.post("roleMenu/queryAllRoleMenu.action",params).then(function (result) {  //成功  执行then里面的方法
              for(var i=0;i<result.data.length;i++){
                _this.roleMenu.push(result.data[i].mid);
              };
              _this.roleid=row.id;
              _this.rname=row.name
          ...
          author(){
            ...
            this.$axios.post("roleMenu/author.action",params).then(function (result) {  //成功  执行then里面的方法
              _this.$message({
                message: result.data,
                type: 'success'
              });
