...
<script>
  export default {
    ...
    methods:{
      getstaff() {  //获取数据
        var _this = this;
        this.$axios.post("staff/queryAll.action").then(function (result) {  //成功  执行then里面的方法
      ...
      getdata() {  //获取数据
        var _this = this;
        this.$axios.post("role/queryfenpeirole.action").then(function (result) {  //成功  执行then里面的方法
      ...
      getStaffRole(row,column,event){
        ...
        params.append("staffid",row.id);
        this.$axios.post("staffRole/queryAllStaffRole.action",params).then(function (result) {  //成功  执行then里面的方法
          for(var i=0;i<result.data.length;i++){
            _this.staffrole.push(result.data[i].id);
          };
          _this.staffid=row.id;
      ...
      fenpei(){
        ...
        this.$axios.post("staffRole/fenpei.action",params).then(function (result) {  //成功  执行then里面的方法
          _this.$message({
            message: result.data,
            type: 'success'
          });
